...
/// Constructor that instantiates an Outatime clock
/// 
/// @param [identifier]   Unique name for this clock. Outatime_CURRENT_CLOCK will be set to this value when the clock's .Update() method is called. Defaults to <undefined>
/// 
/// 
/// 
/// Outatime clocks have the following public methods:
/// 
///   .Update()
...
///	 Adds a variable to be smoothly interpolated between Outatime ticks. The interpolated value is passed to the given output variable name
...
///	 a clock tick method using OutatimeGetInput(). .DefineInput() should be used for "continuous"
...
///	 Outatime alarms respect pausing - N.B. Changing a clock's update frequency will cause alarms to desynchronise
///   
...
///   
///   .GetRemainder()
///	 Returns the remainder on the accumulator (0-1 range for interpolation)
///

function OutatimeClock(_identifier = undefined) constructor
{
	...
	__paused		  = false;
	__microsecondsPerTick = floor(1000000 / __updateFrequency);
	__accumulator	 = 0; // In microseconds
	...
	{
		Outatime_CURRENT_CLOCK = __identifier;
		_outatime.__currentClock = self;
		...
		//delta_time is already in microseconds - no conversion needed
		var _delta = min(floor(1000000 / Outatime_MINIMUM_FRAMERATE), delta_time);
		
		//Start off assuming this clock isn't going to want to process any ticks whatsoever
		Outatime_TICKS_FOR_CLOCK = 0;
		...
		if (!__paused)
		{
			...
			Outatime_TICKS_FOR_CLOCK = __accumulator div __microsecondsPerTick;
			__accumulator -= Outatime_TICKS_FOR_CLOCK * __microsecondsPerTick;
		}
		
		if (Outatime_TICKS_FOR_CLOCK > 0)
		{
			Outatime_MICROSECONDS_PER_TICK = __microsecondsPerTick;
			Outatime_TICK_INDEX = -1;
			...
			Outatime_TICK_INDEX = 0;
			repeat(Outatime_TICKS_FOR_CLOCK)
			{
				//Capture interpolated variable state before the final tick
				if (Outatime_TICK_INDEX == Outatime_TICKS_FOR_CLOCK-1) __VariablesInterpolateRefresh();
				
				__execute_methods(__outatime_CHILD.__BEGIN_METHOD);
				...
				__execute_methods(__outatime_CHILD.__NORMAL_METHOD);
				__execute_methods(__outatime_CHILD.__END_METHOD);
				
				//Reset momentary input after the first tick
				if (Outatime_TICK_INDEX == 0)
				{
			...
			Outatime_TICK_INDEX = Outatime_TICKS_FOR_CLOCK;
			__ResetInputAll();
		}
		
		//Update our output interpolated variables
		if (!__paused) __VariablesInterpolateUpdate();
		
		var _ticks = Outatime_TICKS_FOR_CLOCK;
		
		//Make sure to reset these macros so they can't be accessed outside of Outatime methods
		Outatime_CURRENT_CLOCK	= undefined;
		Outatime_TICKS_FOR_CLOCK  = undefined;
		Outatime_TICK_INDEX	   = undefined;
		Outatime_MICROSECONDS_PER_TICK = undefined;
		
		_outatime.__currentClock = undefined;
	...
	{
		return __AddMethodGeneric(_method, __outatime_CHILD.__BEGIN_METHOD);
	}
	
	static AddTickMethod = function(_method)
	{
		return __AddMethodGeneric(_method, __outatime_CHILD.__NORMAL_METHOD);
	}
	
	static AddEndTickMethod = function(_method)
	{
		return __AddMethodGeneric(_method, __outatime_CHILD.__END_METHOD);
	}
	...
	static AddTickUserEvents = function(_begin = undefined, _normal = undefined, _end = undefined)
	{
		...
		with(other)
		{
			if (_begin != undefined)
			{
				__outatimeBeginUserEvent = _begin;
				other.AddBeginTickMethod(function() { event_user(__outatimeBeginUserEvent); });
			}
			
			if (_normal != undefined)
			{
				__outatimeTickUserEvent = _normal;
				other.AddTickMethod(function() { event_user(__outatimeTickUserEvent); });
			}
			
			if (_end != undefined)
			{
				__outatimeEndUserEvent = _end;
				other.AddEndTickMethod(function() { event_user(__outatimeEndUserEvent); });
			}
		}
	}
	...
	static DefineInput = function(_inputName, _defaultValue)
	{
		...
		{
			__outatimeTrace("Warning! Input name \"", _inputName, "\" already defined");
		}
		else
		{
	...
	static __VariableInterpolateCommon = function(_inName, _outName, _scope, _is_angle)
	{
		...
		{
			__outatimeError("Scope could not be determined (data type=", typeof(_scope), ")");
		}
		
		var _array = _childData[__outatime_CHILD.__VARIABLES_INTERPOLATE];
		
		if (_array == undefined)
		{
			_array = [];
			_childData[@ __outatime_CHILD.__VARIABLES_INTERPOLATE] = _array;
			array_push(__varInterpolateArray, _childData);
		}
		
		var _i = 0;
		repeat(array_length(_array) div __outatime_INTERPOLATED_VARIABLE.__SIZE)
		{
			...
			_i += __outatime_INTERPOLATED_VARIABLE.__SIZE;
		}
	...
	static __VariableSkipInterpolation = function(_outName, _scope)
	{
		...
		var _variables = _childData[__outatime_CHILD.__VARIABLES_INTERPOLATE];
		var _j = 0;
		repeat(array_length(_variables) div __outatime_INTERPOLATED_VARIABLE.__SIZE)
		{
			if (_variables[@ _j + __outatime_INTERPOLATED_VARIABLE.__OUT_NAME] == _outName)
			{
				_variables[@ _j + __outatime_INTERPOLATED_VARIABLE.__PREV_VALUE] = variable_instance_get(_scope, _variables[_j + __outatime_INTERPOLATED_VARIABLE.__IN_NAME]);
			}
			
			_j += __outatime_INTERPOLATED_VARIABLE.__SIZE;
		}
	}
	...
	{
		return new __outatimeClassAlarm(self, (_time / 1000) * GetUpdateFrequency(), _method);
	}
	
	static AddAlarmTicks = function(_ticks, _method)
	{
		return new __outatimeClassAlarm(self, _ticks, _method);
	}
	...
	{
		return __accumulator / __microsecondsPerTick;
	}
	
	#endregion
	
	#region Rollback
	static ForceUpdateTicks = function(_ticks)
	{
		...
		Outatime_TICKS_FOR_CLOCK = _ticks;
		Outatime_MICROSECONDS_PER_TICK = __microsecondsPerTick;
		Outatime_TICK_INDEX = 0;
		...
		var _result = Outatime_TICKS_FOR_CLOCK;
	...
	static __execute_methods = function(_method_type)
	{
		switch(_method_type)
		{
			case __outatime_CHILD.__BEGIN_METHOD: var _array = __beginMethodArray; break;
			case __outatime_CHILD.__NORMAL_METHOD: var _array = __normalMethodArray; break;
			case __outatime_CHILD.__END_METHOD:   var _array = __endMethodArray;   break;
		}
		...
		repeat(array_length(_array))
		{
			...
			if (_childData[__outatime_CHILD.__DEAD])
			{
				array_delete(_array, _i, 1);
				continue;
			}
			
			var _scope = _childData[__outatime_CHILD.__SCOPE];
			switch(__outatimeScopeExists(_scope, _childData[__outatime_CHILD.__outatime_ID]))
			{
	...
	{
		variable_struct_remove(__childrenStruct, _childData[__outatime_CHILD.__outatime_ID]);
		_childData[@ __outatime_CHILD.__DEAD] = true;
	}
	...
	static __AddMethodGeneric = function(_method, _method_type)
	{
		if (is_numeric(_method))
		{
			...
			{
				__outatimeError("Could not find script index ", _method);
			}
		}
		else if (!is_method(_method))
		{
			__outatimeError("Method was an incorrect data type (", typeof(_method), ")");
		}
	...
	static __GetChildData = function(_scope)
	{
		...
		if (is_numeric(_scope))
		{
			if (_scope < 100000)
			{
				__outatimeError("Method scope must be an instance or a struct, object indexes are not permitted");
			}
		}
		else if (!is_struct(_scope) && !instance_exists(_scope))
		{
			__outatimeError("Method scope must be an instance or a struct, found scope's data type was ", typeof(_scope));
		}
		
		var _childID   = __outatimeEnsureChildID(_scope);
		...
		if (_childData == undefined)
		{
			...
			else
			{
				...
				if (is_numeric(_id) && !is_struct(_scope))
				{
					...
					else
					{
						...
						if (Outatime_CHECK_FOR_DEACTIVATION)
						{
			...
			//Give this scope a unique Outatime ID
			//This'll save us some pain later if we need to add a different sort of method
			variable_instance_set(_scope, Outatime_ID_VARIABLE_NAME, _childID);
			
			//Create a new data packet and set it up
			var _childData = array_create(__outatime_CHILD.__SIZE, undefined);
			_childData[@ __outatime_CHILD.__outatime_ID] = _childID;
			_childData[@ __outatime_CHILD.__SCOPE  ] = (_isInstance? _id : weak_ref_create(_scope));
			_childData[@ __outatime_CHILD.__DEAD   ] = false;
		...
		repeat(array_length(_array))
		{
			...
			switch(__outatimeScopeExists(_scope, _childData[__outatime_CHILD.__outatime_ID]))
			{
				...
				case 2: //Alive struct
					...
					repeat(array_length(_variables) div __outatime_INTERPOLATED_VARIABLE.__SIZE)
					{
						if (_variables[_j + __outatime_INTERPOLATED_VARIABLE.__IS_ANGLE])
						{
							var _oldValue = _variables[_j + __outatime_INTERPOLATED_VARIABLE.__PREV_VALUE];
							var _newValue = _oldValue + _remainder*angle_difference(variable_instance_get(_scope, _variables[_j + __outatime_INTERPOLATED_VARIABLE.__IN_NAME]), _oldValue);
						}
						else
						{
							var _newValue = lerp(_variables[_j + __outatime_INTERPOLATED_VARIABLE.__PREV_VALUE], variable_instance_get(_scope, _variables[_j + __outatime_INTERPOLATED_VARIABLE.__IN_NAME]), _remainder);
						}
						
						variable_instance_set(_scope, _variables[_j + __outatime_INTERPOLATED_VARIABLE.__OUT_NAME], _newValue);
