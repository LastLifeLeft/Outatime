...
function __outatimeScopeExists(_scope, _expectedChildID) //Does do deactivation check
{
	if (is_numeric(_scope))
	{
		...
		{
			if ((_expectedChildID != undefined) && (variable_instance_get(_scope, Outatime_ID_VARIABLE_NAME) != _expectedChildID))
			{
				return -3;
			}
			else
