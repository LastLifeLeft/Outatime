...
/// 
/// The scope is determined by who calls OutatimeSkipInterpolation(). Occasionally you may need to force
...
function OutatimeSkipInterpolation()
{
	static _outatime = __outatime();
	
	if (_outatime.__currentClock == undefined) __outatimeError("Cannot use OutatimeSkipInterpolation() outside of a clock method");
	
	var _outName = argument[0];
	var _scope	= ((argument_count > 1) && (argument[1] != undefined))? argument[1] : self;
	
	_outatime.__currentClock.__VariableSkipInterpolation(_outName, _scope);
}
